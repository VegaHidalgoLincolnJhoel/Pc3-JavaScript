...
class UtpRiskRequest(BaseModel):
    ...
    horas_study_semana: float = Field(..., ge=0, le=40)
...
def recommendation_utp(payload: UtpRiskRequest, label: str) -> List[str]:
    ...
    if payload.horas_study_semana < 6:
...
def startup_train_models():
    ...
    MODELS["utp-risk"] = train_pack(
        ["promedio_actual", "asistencia_pct", "tareas_entregadas_pct", "participacion_pct", "horas_study_semana", "nota_pc_anterior"], X, y
    )
